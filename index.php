...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    function bind($qq, $playerName, $activationCode) {
        ...
        // 防止 SQL 注入攻击
        $stmt = $conn->prepare("SELECT * FROM activation_codes WHERE activation_code=? AND status=0");
        ...
        if ($result && $result->num_rows > 0) {
            // 激活码存在且状态为0，更新状态为1
            $stmt = $conn->prepare("UPDATE activation_codes SET status=1 WHERE activation_code=?");
            $stmt->bind_param("s", $activationCode);
            $stmt->execute();
            $stmt->close();
        } else {
        ...
        if ($existingRecord) {
            ...
            $newExpiredTime = $existingExpiredTime + 60 * 60 * 24 * $duration; // 新的过期时间
            ...
            // 不存在未过期的记录，进行插入操作
            $duration = explode("-", $activationCode)[2];
            $expiredTime = time() + 60 * 60 * 24 * $duration;
